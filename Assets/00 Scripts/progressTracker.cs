    ...
    private void PerformStateActions()
    {
        switch (currentState)
        {
            ...
            case LabState.Step3:
                ...
                {
                    if (obj.transform.name.StartsWith("Erlenmeyer Flask 250")){
                        if (obj.GetComponent<liquidScript>().liquidTemperature > 355.15f && obj.GetComponent<liquidScript>().currentVolume_mL > 20f && obj.GetComponent<liquidScript>().percentKAlOH4 > 0.35f){
                            step1Erlenmeyer = obj;
                            TransitionToNextState();
                        }
                    }
